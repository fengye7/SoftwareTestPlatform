<template>
  <div class="console-container">
    <div class="left-card">
      ...
      <p class="text-p">
        实现方式: 使用 Maven 执行集成测试，实时通过 WebSocket 输出测试信息。
      </p>
      <p class="text-p">
    ...
    <div class="right-card">
      ...
      <div class="button-container">
        <div class="left-button-group">
          <el-button @click="runIntegrationTest" type="primary"
            >运行集成测试</el-button
          >
        </div>
        <div class="right-button-group">
          <el-button @click="terminateIntegrationTest" type="danger">终止集成测试</el-button>
...
onMounted(async () => {
  try {
    const response = await fetch(`${baseURL}/get-integration-test-report`);
...
//   console.log("Starting integration test...");
  ...
  socket.onopen = async () => {
    ...
    try {
      const response = await fetch(`${baseURL}/run-integration-test`);
      const result = await response.text();
      ElMessage.success(result);
      isTestSuccess.value = "测试成功";
    //   console.log("Integration test started successfully.");
    } catch (error) {
...
// 终止测试
const terminateIntegrationTest = async () => {
//   console.log("Terminating Integration test...");
  try {
    ...
    const result = await response.text();
    ElMessage.success(result);
    // console.log("Integration test terminated successfully.");
    if (socket) {
      socket.close();
    }
  } catch (error) {
...
const downloadTestReport = () => {
  ...
  a.href = url;
  a.download = "integration-test-report.txt";
...
.console-container {
  ...
  padding: 20px;
  gap: 20px;
  background-color: rgb(201, 235, 252);
}
